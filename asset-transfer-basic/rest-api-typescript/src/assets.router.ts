...
assetsRouter.put(
    ...
    async (req: Request, res: Response) => {
        ...
        if (req.params.assetId != req.body.identifier) {
            return res.status(BAD_REQUEST).json({
                status: getReasonPhrase(BAD_REQUEST),
...
assetsRouter.post(
    '/:assetId/lend',
    body('lendee', 'lendee must be a non-empty string').notEmpty(),
    async (req: Request, res: Response) => {
        logger.debug(req.body, 'Lend asset request received');
        ...
        try {
            ...
            logger.error(
                { err },
                'Error processing lend asset request for asset ID %s',
                assetId
            );
            return res.status(INTERNAL_SERVER_ERROR).json({
                status: getReasonPhrase(INTERNAL_SERVER_ERROR),
...
assetsRouter.post('/:assetId/return', async (req: Request, res: Response) => {
    logger.debug(
        'Return asset request received for asset ID %s',
        req.params.assetId
    );
    ...
    try {
        ...
        logger.error(
            { err },
            'Error processing return asset request for asset ID %s',
            assetId
        );
        return res.status(INTERNAL_SERVER_ERROR).json({
            status: getReasonPhrase(INTERNAL_SERVER_ERROR),
